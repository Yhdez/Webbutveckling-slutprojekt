...
var Lista_av_Titlar_resultat = [] //Detta är en lista som kommer att innehålla de resultat som visas efter din sökning
...
Sök_bar.onkeydown = async function(event){ //Om användaren går in på sök baren för att söka efter en film
    if (event.key === "Enter"){ //När användaren trycker på enter efter att ha skrivit något inom input fältet (Sök_bar)
        ...
        await Create_results(Resultat.results) //När sök_efter_film() är klar startas en ny funktion som är tänkt att uttnytja informationen given från requesten från sök_efter_film(). Genom att filtrera bort resultat, lägga in dem i en div och skapa interaktion med divarna
        ...
        async function sök_efter_film(Film_sökt){
            ...
            let json = await response.json(response) //När requesten har fått en response tar den emot informationen och sammlar den i en variabel (json)
        ...
        async function Create_results(Resultat){
          ...
          for(let object = 0; object < Resultat.length; object++){ /*Går igenom varje titel som var given av themoviedatabase, en i taget*/
...
function Show_information_on_result(Resultat){ //Gör om sökreslutat fönstret, för att instället ge information om titeln som var tryckt
  ...
  <div id="Information_text_om_titel">
